...
def reduce_dimensions_40():
    # Conectar a Neo4j
    print("Conectando a Neo4j...")
    driver = GraphDatabase.driver(NEO4J_URI, auth=(NEO4J_USER, NEO4J_PASSWORD))
    
    # Crear carpeta reducers_40 si no existe
    os.makedirs("reducers_40", exist_ok=True)
    print("Carpeta 'reducers_40/' creada o ya existe")
        ...
        with driver.session() as session:
            ...
            for cluster_id in sorted(clusters_data.keys()):
                try:
                    ...
                    # Verificar tamaño mínimo del clúster (ahora 40 en lugar de 20)
                    if n_samples < 40:
                        print(f"⚠️  Clúster {cluster_id}: Solo {n_samples} noticias (ajustando k_features). Continuando...")
                    ...
                    # Aplicar SelectKBest con 40 características (o menos si no hay suficientes muestras)
                    k_features = min(40, n_samples - 1, embeddings.shape[1] // 2)  # Máximo 40, n_samples-1, o la mitad de dimensiones
                    ...
                    # Guardar el selector en carpeta reducers_40
            ...
            # Verificar archivos guardados en reducers_40
            saved_files = [f for f in os.listdir("reducers_40") if f.startswith("selector_cluster_")]
            print(f"   Archivos guardados en 'reducers_40/': {len(saved_files)}")
            
            if saved_files:
                print("   📁 Archivos creados en reducers_40/:")
                for file in sorted(saved_files):
                    print(f"      - {file}")
            
            # Mostrar distribución por clúster
            print(f"\n📊 Distribución final por clúster:")
            with driver.session() as stats_session:
                for cluster_id in sorted(clusters_data.keys()):
                    count_result = stats_session.run("""
                        MATCH (n:Noticia)
                    ...
                    n_populares = np.sum(clusters_data[cluster_id]["popularities"]) if cluster_id in clusters_data else 0
                    n_no_populares = count - n_populares
                    print(f"   Clúster {cluster_id}: {count} noticias ({n_populares} populares, {n_no_populares} no populares)")
            
            print("\n✅ Proceso de reducción de dimensiones (40 características) completado!")
            print("✅ Confirmación de actualización en Neo4j: Clústeres ya actualizados por cluster_news_5.py")
